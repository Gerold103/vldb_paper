...
tuples. But it is not possible for secondary indexes. If a table contains
secondary indexes, any update operation must read an old tuple from a primary
index, extract from it secondary keys and delete them from each secondary index
explictly. That is any update of such table produces hidden reads from a primary
index.

Primary index read and explicit old tuple deletion can not be avoided, because
LSM-tree deletes old versions of the key, only if these versions are equal by
the key. If a request changes a secondary key in an exising tuple and does not
deletes old one, then it is not deleted from a secondary index, becuase LSM-tree
considers these two records to have different keys.

% A primary index stores tuples as they were inserted by an user - all columns are
% stored. A secondary index stores only its key columns, by which the index is
% sorted, and primary index columns. It saves memory because secoundary index
% count can be big, and storage of entire copy of each tuple in each index would
% be very expensive.


% \begin{figure}
% \centering
% \centerline{\includegraphics[width=0.5\textwidth]{sk_record}}
% \caption{Secondary index and request structure structure}
% \label{fig:sk_record}
% \end{figure}
% If an user's request contains a condition allowing to use a secondary index's
% sorting order, then in the secondary index the search is executed followed by a
% search in a primary index by a primary key from the found tuple. It is where the
% primary columns from a secondary index's tuple are used - they forms a link to
% the full record in a primary index.
